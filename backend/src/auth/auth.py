...
AUTH0_DOMAIN = 'dev-ol-7qy4l.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    # Get the value of the Authorization header, or None.
    auth = request.headers.get('Authorization', None)
    # Raise AuthError if Authorization header does not exist.
    if not auth:
        raise AuthError({
            'code': 'authorization_header_missing',
            'description': 'Authorization header is expected.'
        }, 401)
    # Split the text parts.
    parts = auth.split()
    # Check if valid bearer or raise an AuthError.
    if parts[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must start with "Bearer".'
        }, 401)
    # Check if the token exists or raise an AuthError.
    elif len(parts) == 1:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Token not found.'
        }, 401)
    # Check the type of authorization, must be bearer token,
    # or raise an AuthError.
    elif len(parts) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must be bearer token.'
        }, 401)
    # Auth header value is as expected, Get the token.
    token = parts[1]
    # Return the token.
    return token
